...
object V2exHtmlUtil {
    ...
    fun getLoginValue(response: String): LoginKey {
        ...
        val formTableBody =
            body.getElementsByAttributeValue("action", "/signin").select("tbody").select("tr")
        val userNameValue =
            formTableBody[0].getElementsByAttributeValue("type", "text").attr("name")
    ...
    fun getMainPageItems(response: String): List<MainPageItem> {
        ...
        for (item in content) {
            ...
            val commentCount = if (item.getElementsByClass("count_livid").select("a[href]").text()
                    .isNullOrBlank()
            ) {
                "0"
            } else {
    ...
    fun getDailyMissionParam(response: String): String {
        val doc = Jsoup.parse(response)
        val main = doc.getElementById("Main")
        val button = main.getElementsByAttributeValueMatching(
            "onclick",
            "location.href = '/mission/daily/redeem\\?once=[0-9]+';"
        )
